            ...
            void gensboxes() {
                ...
                loop(pass, 5) loopi(256) for (int sb = 0; sb < 1024; sb += 256) {
                            ...
                            loop(col, 8) {
                                ...
                                ((uchar*) &sboxes[sb + i])[col] = ((uchar*) &sboxes[sb +
                                                                                    ((uchar*) &state[abc])[col]])[col];
...
#define BI_DIGIT_MASK ((1<<BI_DIGIT_BITS)-1)
        ...
        struct bigint {
            ...
            bigint(const bigint<Y_DIGITS>& y) { *this = y; }
            ...
            void printdigits(std::vector<char>& buf) const {
                loopi(len) {
                    ...
                    loopj(BI_DIGIT_BITS / 4) {
                        ...
                        int val = (d >> shift) & 0xF;
                        if (val < 10) (buf.emplace_back('0' + val), buf.back());
                        else (buf.emplace_back('a' + val - 10), buf.back());
                    }
                }
            }

            template<int Y_DIGITS>
            bigint& operator=(const bigint<Y_DIGITS>& y) {
                len = y.len;
            ...
            template<int Y_DIGITS>
            bigint& add(const bigint<Y_DIGITS>& y) { return add(*this, y); }
            ...
            bigint& sub(const bigint<X_DIGITS>& x, const bigint<Y_DIGITS>& y) {
                ...
                for (i = 0; i < y.len || borrow; i++) {
                    borrow = (1 << BI_DIGIT_BITS) + (dbldigit) x.digits[i] - (i < y.len ? (dbldigit) y.digits[i] : 0) -
                             borrow;
            ...
            template<int Y_DIGITS>
            bool operator>(const bigint<Y_DIGITS>& y) const { return y < *this; }
 ...
 */
        struct gfield : gfint {
            ...
            template<int Y_DIGITS>
            gfield& operator=(const bigint<Y_DIGITS>& y) {
                gfint::operator=(y);
                return *this;
            }

            template<int X_DIGITS, int Y_DIGITS>
            gfield& add(const bigint<X_DIGITS>& x, const bigint<Y_DIGITS>& y) {
                gfint::add(x, y);
            ...
            template<int Y_DIGITS>
            gfield& add(const bigint<Y_DIGITS>& y) { return add(*this, y); }

            template<int X_DIGITS>
            gfield& mul2(const bigint<X_DIGITS>& x) { return add(x, x); }
...
#error Unsupported GF
        ...
        struct ecjacobian {
            ...
            void add(const ecjacobian& q) {
                ...
                if (a.iszero()) {
                    if (b.iszero()) mul2(); else *this = origin;
                    return;
                }
        ...
        const ecjacobian ecjacobian::origin(gfield((gfield::digit) 1), gfield((gfield::digit) 1),
                                            gfield((gfield::digit) 0));
