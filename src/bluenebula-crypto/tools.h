...
#define BIGSTRLEN 4096
    ...
    inline void vformatstring(char* d, const char* fmt, va_list v, int len) {
    ...
    inline char* concatstring(char* d, const char* s, size_t len) {
    ...
    template<size_t N>
    inline char* prependstring(char (& d)[N], const char* s) { return prependstring(d, s, N); }
...
#define loopvkrev(v) for(int k = (v).length()-1; k>=0; k--)
    ...
    struct isclass {
        ...
        enum {
            yes = sizeof(test<T>(0)) == 1 ? 1 : 0, no = yes ^1
        };
    };
    ...
    struct vector {
        ...
        void shrink(int i) {
            ASSERT(i <= ulen);
            if (isclass<T>::no) ulen = i; else while (ulen > i) drop();
        }
...
#define new new(__FILE__,__LINE__)
    ...
    template<>
    inline double endianswap<double>(double n) {
        union {
            double t;
            uint i;
        } conv;
    ...
    template<class T>
    inline void endiansame(T* buf, size_t len) {}
...
#undef putchar
    ...
    struct stream {
        ...
        template<class T>
        bool putlil(T n) { return put<T>(lilswap(n)); }

        template<class T>
        bool putbig(T n) { return put<T>(bigswap(n)); }
        ...
        template<class T>
        T getlil() { return lilswap(get<T>()); }

        template<class T>
        T getbig() { return bigswap(get<T>()); }
    };
    ...
    extern char*
    makefile(const char* s, const char* e = "", int revision = 0, int start = 1, bool store = false, bool skip = false);
    ...
    extern stream*
    opengzfile(const char* filename, const char* mode, stream* file = NULL, int level = Z_BEST_COMPRESSION);
    ...
    extern void
    backup(const char* fname, const char* ext, int revision = 0, int start = 1, bool store = false, bool full = true);
    ...
    template<size_t N>
    static inline void getstring(char (& t)[N], ucharbuf& p) { getstring(t, p, N); }
